...
public class OotdController {
    ...
    @ApiOperation(value = "Ootd 메인페이지 (O)",notes = "[#Ootd메인 페이지] 0 혹은 1값을 보내면 0:최신순 1: 인기순으로 리턴해준다(ootdIdx,닉네임,글내용,해시태그,좋아요 개수)")//안됨
    ...
    @ApiOperation(value = "Ootd 상세페이지 (X)",notes = "Ootd글에 대한 상세페이지, ootd메인페이지에서 특정 글을 클릭시" +
...
//        Ootd ootdDetail = ootdRepository.findByIdx(ootdidx);
    ...
    @PutMapping("/ootd")
    @Transactional
    @ApiOperation(value = "Ootd 글수정 (O)",notes = "Ootd상세 페이지에서 글 수정.수정할 글의 ootdidx, 해시태그, 글 제목(내용)을 넘겨주면 수정한다.")//이건 된다.
